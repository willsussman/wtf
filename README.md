...
`vitals2bits()` applies a simple signal-processing scheme: For $j$th sample of vital $i$, if $y_ij R \beta_i * EWMA(\alpha_i, y_ij)$, then the bit is set. Else, the bit is unset. We do not claim that this scheme is optimal; improving it is future work.
